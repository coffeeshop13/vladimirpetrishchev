...
.header-links { align-items:center; gap:26px; font-size:14px; display:flex; } .header-links a:not(.header-contact) { opacity:.72; } .header-links a:hover { opacity:1; }
...
.post-grid { border-top:1px solid var(--ink); margin-top:100px; } .post-card { border-bottom:1px solid var(--line); grid-template-columns:18% 45% 37%; gap:20px; padding:26px 0; display:grid; transition:background .2s; } .post-card:hover { background:#eeece5; } .post-card time,.post-meta { color:#6a7077; padding-top:5px; font-family:DM Mono,monospace; font-size:11px; } .post-card h2 { letter-spacing:-.04em; margin:0 0 8px; font-family:Fraunces,serif; font-size:29px; font-weight:500; line-height:1.12; } .post-card p { color:#515963; max-width:340px; margin:2px 0; font-size:14px; line-height:1.5; } .tags { color:var(--blue); font-family:DM Mono,monospace; font-size:11px; padding-top:5px; }
...
for (const { metadata, html } of posts) {
  ...
  const body = `<header class="site-header"><a class="wordmark" href="/" aria-label="Vladimir Petrishchev home">VP<span>·</span></a><nav class="header-links" aria-label="Blog navigation"><a href="/blog/index.html">All notes</a><a href="/rss.xml">RSS</a></nav></header><main class="article-shell"><a class="back-link" href="/blog/index.html">← All notes</a><article><header class="article-header"><div class="eyebrow">${(metadata.tags || []).map(escapeHtml).join(" · ")}</div><div><h1>${escapeHtml(metadata.title)}</h1><div class="post-meta"><time datetime="${escapeHtml(metadata.date)}">${escapeHtml(metadata.date)}</time></div><p>${escapeHtml(metadata.description || "")}</p></div></header><div class="article-body">${html}</div></article></main>${footer}`;
